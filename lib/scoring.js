...
export function scoreProperty(listing, market, news, weights = DEFAULT_WEIGHTS) {
  const signals = {};
  // Floor avgDOM at 20 — guards against corrupt/missing market data producing
  // false distress signals on fresh listings. No real market has median DOM < 20.
  const avgDOM = Math.max(20, market?.medianDOM || 45);
  const domRatio = listing.dom / avgDOM;
  // Explicitly zero for listings < 1 day old; otherwise only score above-average DOM
  signals.dom = listing.dom < 1 ? 0 : Math.min(25, Math.round(domRatio > 1 ? (domRatio - 1) * 20 : 0));

  // Filter out past-sale events before computing cuts/startPrice — prevents a 1993
  // sold record at $80k from being treated as the listing start price.
  const listingHistory = (listing.priceHistory || []).filter(h => h.event !== "Sold");
  const cuts = listing.priceDropCount != null
    ? listing.priceDropCount
    : Math.max(0, listingHistory.length - 1);
  const startPrice = listing.listingStartPrice || listingHistory[0]?.price || listing.price;
  const totalCutPct = startPrice > listing.price ? (startPrice - listing.price) / startPrice : 0;
  ...
  return {
    ...
    pricecuts: cuts,
    totalCutPct: Math.round(totalCutPct * 100),
  };
}
